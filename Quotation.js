import React from "react";
import {BrowserRouter as Router, Routes, Switch, Link} from 'react-router-dom';
import '../App.css';
import QuoteResult from "./QuoteResult";

const Quotation = () => {

  return (
    <div>
      <div className="quotation">
            <div className="App">
            <header className="App-header">
            <a>
              <img src="data:image/jpeg;base64,">
              </img>
            </a>
            <h1>Quotation</h1>
            <h2>Please input your details below:</h2>
            <br>
            </br>
            <form className="quotation-form">
             <div className="form-group">
              <label>Name of buyers/sellers </label>
              <input type="nameofbors" className="nameofbors" />
             </div>
             <div className="form-group-options">
              <label>Sole or joint application </label>
              <br />
              <select>
               <option value="">--Please select an option--</option>
               <option value="sole">Sole</option>
               <option value="joint">Joint</option>
              </select> 
             </div>
             <div className="form-group">
              <label>Telephone number </label>
              <input type="phonenumber" className="phonenumber" />
             </div>
             <div className="form-group">
              <label>Email address </label>
              <input type="emailaddress" className="emailaddress" />
             </div>
             <div className="form-group">
              <label>Address</label>
              <input type="address" className="postcode" />
             </div>
             <div className="form-group">
              <label>Postcode </label>
              <input type="postcode" className="postcode" />
             </div>
             <div className="form-group">
              <label>Sale/purchase price </label>
              <input type="price" className="price" />
             </div>
             <div className="form-group-options">
              <label>Tenure </label>
              <br />
              <select>
               <option value="">--Please select an option--</option>
               <option value="freehold">Freehold</option>
               <option value="leasehold">Leasehold</option>
              </select> 
             </div>
             <div className="form-group-options">
              <label>Is it a purchase matter? </label>
              <br />
              <select>
               <option value="">--Please select an option--</option>
               <option value="no" purchaseMatter="false">No</option>
               <option value="yes" purchasematter="true">Yes</option>
              </select> 
             </div>
             <div className="form-group-options">
              <label>Is a remortgage needed? </label>
              <br />
              <select>
               <option value="">--Please select an option--</option>
               <option value="no" remortgage="false">No</option>
               <option value="yes" remortgage="true">Yes</option>
              </select> 
             </div>
             <div className="form-group-options">
              <label>Is it a transfer of equity? </label>
              <br />
              <select>
               <option value="">--Please select an option--</option>
               <option value="no" equity="false">No</option>
               <option value="yes" equity="true">Yes</option>
              </select> 
             </div>
             <h3>Only answer the following questions if you answered yes to the previous question:</h3>
             <div className="form-groups">
              <label>New build? </label>
              <br />
              <select >
               <option value="">--Please select an option--</option>
               <option value="yes">Yes</option>
               <option value="no">No</option>
              </select> 
              <br /> 
              <label>First time buyers? </label>
              <br />
              <select >
                <option value="">--Please select an option--</option>
                <option value="yes">Yes</option>
                <option value="no">No</option>
              </select> 
              <br />
              <label>Mortgage taken out? </label>
              <br />
              <select >
                <option value="">--Please select an option--</option>
                <option value="yes">Yes</option>
                <option value="no">No</option>
              </select> 
              <br />
              <label>Using a Help to Buy/Lifetime ISA? </label>
              <br />
              <select >
                <option value="">--Please select an option--</option>
                <option value="no">No</option>
                 <option value="HtoB">Help to Buy</option>
                 <option value="lifetimeISA">Lifetime ISA</option>
              </select> 
              <br />
              <label>Main residence or additional? </label>
              <br />
              <select >
                <option value="">--Please select an option--</option>
                <option value="mainresidence">Main residence</option>
                <option value="additional">Additional</option>
              </select> 
              <br />
              <label>UK resident for past 183 days? </label>
              <br />
              <select >
                <option value="">--Please select an option--</option>
                <option value="yes">Yes</option>
                <option value="no">No</option>
              </select> 
              </div>
              <br />
                  <Link>
                    <button>Submit</button>
                  </Link>  
            </form> 
            </header> 
        </div>
    </div>
    </div>
  );
};
  
export default Quotation;