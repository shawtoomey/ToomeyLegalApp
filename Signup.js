...
const SignUp = (props) => {
  ...
  const [forenameReg, setForenameReg] = useState("")
  const [surnameReg, setSurnameReg] = useState("")
  const [emailReg, setEmailReg] = useState("")
  const [passwordReg, setPasswordReg] = useState("")

  const signUp = () => {
    Axios.post('http://localhost:3000/signup', {forename: forenameReg, surname: surnameReg, email: emailReg, password: passwordReg
   }).then((response) => {
    ...
    <div className='SignUp'>
      <div className="App">
       <header className="App-header">
        ...
        <h1>Sign up to the Toomey Legal app</h1>
        ...
        <form className="Sign-Up-Form">
         ...
         <div className="form-group">
          ...
          <input type="text" className="forename" onChange={(e) => {
            setForenameReg(e.target.value);
          }}/>
         </div>
         <div className="form-group">
          <label>Surname </label>
          <input type="text" className="surname" onChange={(e) => {
         ...
         <div className="form-group">
          ...
          <input type="passwordconfirm" className="passwordconfirm" />
         </div>
         <br />
         <button type="submit" value="send" onClick={signUp}>Submit</button>
         <a href="http://localhost:3000/Login">Already got an account?</a>
        </form>
       </header>
      </div>
    </div>
